...
_RA_STATUS Linux_DHCPPoolsForEntity_getResources( const CMPIBroker * broker, const CMPIContext * context, const CMPIObjectPath * reference, _RESOURCES ** resources)
{
	...
	for(i = 0; i < rhs_length; i++)
	{
	    ...
	    if (lhs_op && rhs_op && (state = Linux_DHCPPoolsForEntity_isAssociated(lhs_op, rhs_op))) {
...
_RA_STATUS Linux_DHCPPoolsForEntity_getResourceForObjectPath( const CMPIBroker * broker, const  CMPIContext * ctx, _RESOURCES * resources, _RESOURCE ** resource, const CMPIObjectPath * objectpath)
{
	...
	if (cmpi_status.rc != CMPI_RC_OK) {
	    ...
	    //ra_status.rc = RA_RC_FAILED;
	    //ra_status.messageTxt = strdup("Failed to get InstanceID");
	    return ra_status;
	}
	...
	if(strcasecmp(src,dest) == 0) {
	    ...
	    ra_status.rc = RA_RC_FAILED;
	    //ra_status.messageTxt = strdup("Failed to get resource");
	    (*resource) = NULL;
	}
    }
    return ra_status;
