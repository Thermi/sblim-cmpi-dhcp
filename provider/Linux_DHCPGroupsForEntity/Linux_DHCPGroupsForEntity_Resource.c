...
int Linux_DHCPGroupsForEntity_isAssociated ( CMPIObjectPath * lhs, CMPIObjectPath * rhs)
{
    ...
    lchild = ra_tokenize((char*)cmpi_namer);
    ...
    rchild = ra_tokenize((char*)cmpi_namel);
...
_RA_STATUS Linux_DHCPGroupsForEntity_getResources( const CMPIBroker * broker, const CMPIContext * context, const CMPIObjectPath * reference, _RESOURCES ** resources)
{
	...
	for(i = 0; i < rhs_length; i++)
	{
	    ...
	    if (lhs_op && rhs_op && (state = Linux_DHCPGroupsForEntity_isAssociated(lhs_op, rhs_op))) {
